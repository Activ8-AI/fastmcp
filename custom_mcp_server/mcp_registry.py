...
def get_servers_by_use_case(use_case: UseCase) -> list[MCPServerConfig]:
    ...
    return [
        server for server in MCP_SERVERS.values() if use_case in server.use_cases and server.enabled
    ]
...
def build_multi_server_config(server_names: list[str] | None = None) -> dict[str, Any]:
    ...
    return {
        "mcpServers": {
            name: config.to_client_config() for name, config in servers.items() if config.enabled
        }
    }
