...
SECRETS_REGISTRY: dict[str, ServiceSecrets] = {
    ...
    "anthropic": ServiceSecrets(
        ...
        secrets=[
            ...
            SecretDefinition(
                "ANTHROPIC_API_KEY_FOR_CI", "Anthropic API key for CI workflows", required=False
            ),
        ],
    ),
    # -------------------------------------------------------------------------
...
def get_secret_required(name: str) -> str:
    ...
    if value is None:
        raise ValueError(
            f"Required secret '{name}' not found. Set it as an environment variable or in .env"
        )
    return value
